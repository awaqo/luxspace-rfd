  ...
  const fnCheckIndex = useCallback(
    (e) => {
      if (e.propertyName === "left") {
        setTimeout(() => {
          removeClass(refDragHandler.current, "transition-all druation-200");
        }, 200);
    ...
    [cardCount, itemToShow, index, cardSize]
  );

  const fnShiftItem = useCallback(
    (direction) => {
      console.log(direction);
      addClass(refDragHandler.current, "transition-all druation-200");
